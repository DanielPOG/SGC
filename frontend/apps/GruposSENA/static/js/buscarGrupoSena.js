document.addEventListener("DOMContentLoaded", () => {
  ...
  const inputBuscar = document.getElementById("buscarGrupo");
  const btnBuscar = document.getElementById("btnBuscar");

  // Elementos de modales globales
  ...
  // Modal dinámico para añadir funcionario
  ...
  if (!tbody) {
    console.warn("No se encontró el tbody con id 'tabla-grupos'");
    return;
  }
  ...
  async function cargarGrupos(search) {
    ...
    try {
      const url = `http://127.0.0.1:8001/api/gruposena/grupo-sena/?search=${encodeURIComponent(
        search
      )}`;
  ...
  // 🔹 Modales dinámicos con clase open-nuevo-modal
  function activarModalesNuevos() {
    document.querySelectorAll(".open-nuevo-modal").forEach((btn) => {
      btn.addEventListener("click", (e) => {
        e.preventDefault();
        const grupoId = btn.getAttribute("data-id"); // Aquí obtienes el grupo seleccionado
        console.log("Abrir modal para grupo:", grupoId);

        if (nuevoModal) {
          nuevoModal.dataset.grupoId = grupoId; // Guarda el id en el modal
          nuevoModal.classList.remove("hidden");
        }
      });
    });

    closeNuevoModal?.addEventListener("click", () => nuevoModal?.classList.add("hidden"));
    cancelNuevoModal?.addEventListener("click", () => nuevoModal?.classList.add("hidden"));
  }
  ...
  inputBuscar?.addEventListener("keypress", (e) => {
    if (e.key === "Enter") {
      e.preventDefault();
      cargarGrupos(inputBuscar.value.trim());
    }
  });
  ...
  // 🔹 Inicialización de modal Excel
  openModal?.addEventListener("click", () => modalExcel?.classList.remove("hidden"));
  closeModal?.addEventListener("click", () => modalExcel?.classList.add("hidden"));

  fileInput?.addEventListener("change", (e) => {
    fileName.textContent = e.target.files[0]?.name || "Ningún archivo";
  });
});
