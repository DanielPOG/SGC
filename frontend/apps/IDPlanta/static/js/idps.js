  ...
  export function idpRow(idp, cargos) {
    ...
    const asignCyR = ()=>{
      apiFetch('http://127.0.0.1:8001/api/cargos/idps/asignarCargo').then(res=>res.json()).then(
        data => {
         if(data.error || !data){
            Swal.fire({
            title: 'Error!',
...
`}</div>
  ...
  export async function cargarIdps(cargos) {
    try {
      ...
      const resR = await apiFetch('http://127.0.0.1:8001/api/general/regionales/')
    ...
    modal.innerHTML = `
      <div class="bg-white rounded-lg shadow-lg w-96 p-6 m-auto mt-36 relative">
        ...
        <select id="cargoSelect-${idpNumero}" class="w-full border rounded-lg p-2 mb-4">
          ...
          ${window.cargos.map(c => `<option value="${c.id}">${c.cargoNombre.nombre}</option>`).join("")}
        </select>
        <h2 class="text-xl font-bold mb-4">Asignar Regional</h2>

        <select id="regionalSelect-${window.regionales.id}" class="w-full border rounded-lg p-2 mb-4">
          <option value="">Seleccione...</option>
          ${window.regionales.map(c => `<option value="${c.id}">${c.nombre}</option>`).join("")}
        </select>
        <button 
